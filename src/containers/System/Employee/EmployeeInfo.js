...
const EmployeeInfo = (props) => {
    ...
    const getDetailUser = async () => {
        let res = await fetchUserDetail(props.userRedux.id)
        if(res && res.EC === 0) {
            let imageURL = ''
            if(res.DT.image) {
    ...
    const handleUpdate = async () => {
        ...
        if(check === true) {
            ...
            setUserData({...newData, id: props.userRedux.id})
            let res = await fetchUpdateUser (userData)
            if(res && res.EC === 0) {
    ...
    return(
        <>
        <div className='Detail-doctor DB'>
